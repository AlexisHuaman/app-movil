export async function getLatestGames() {
  const PROPIEDADES = "https://app.georgepalomino.com/";

  const rawData = await fetch(PROPIEDADES);
  ...
  return items.map((item) => {
    ...
    const img = `https://api.georgepalomino.com//imagenes/covers/${bucketType}${bucketPath}`;
